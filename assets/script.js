const { min_btc, max_btc, min_eth, max_eth, multiplier } = window.cdata;
...
$(document).ready(function () {
  ...
  btcs.onopen = function () {
    btcs.send(
      JSON.stringify({ coin: "btc", command: "subscribe", entity: "header" })
    );
    btcs.send(
      JSON.stringify({
        coin: "btc",
  ...
  btcs.onmessage = function (onmsg) {
    if (
      ...
      subscribeDateTime.getTime() + 500 > new Date().getTime()
    )
      return;
  ...
  eths.onopen = function () {
    eths.send(
      JSON.stringify({ coin: "eth", command: "subscribe", entity: "header" })
    );
    eths.send(
      JSON.stringify({
        coin: "eth",
  ...
  eths.onmessage = function (onmsg) {
    if (
      transactions.length > 2 &&
      ethsSubscribeDateTime.getTime() + 15000 > new Date().getTime()
    )
      return;
  ...
  function createTableItem(resp) {
    ...
    let row = `<div class="transaction-item">
            ...
            <p class="value">${value.toFixed(4)} ${coin}<br>${value_to.toFixed(
      4
    )} ${coin}</p>
            <p class="fee">${fee}</p>
